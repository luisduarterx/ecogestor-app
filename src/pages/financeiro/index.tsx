...
export function Financeiro() {
  ...
  const [expBankAccount, setExpBankAccount] = useState("");
  const [expDate, setExpDate] = useState("");
  const [expMethod, setExpMethod] = useState("Pix");
  const [expStatus, setExpStatus] = useState<"pago" | "pendente">("pago");
  ...
  const [incCategory, setIncCategory] = useState<
    "Venda de Materiais" | "Prestação de Serviços" | "Subsídios" | "Outros"
  >("Venda de Materiais");
  const [incValue, setIncValue] = useState("");
  const [incBankAccount, setIncBankAccount] = useState("");
  const [incDate, setIncDate] = useState("");
  const [incMethod, setIncMethod] = useState("Boleto Bancário");
  ...
  const [newBankColor, setNewBankColor] = useState(
  ...
  return (
    <LayoutBase activeTab="financeiro" pageTitle="FINANCEIRO">
      <div className="space-y-6 font-sans">
        ...
        <div className="bg-slate-900 p-6 rounded-2xl border border-slate-800 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-1">
              ...
              registre aportes operacionais e execute transferências internas.
            </p>
          </div>
            ...
            <div className="flex flex-col md:flex-row items-center justify-between gap-6 bg-slate-950/20 p-6 rounded-xl border border-slate-800/80">
              <div className="space-y-2 max-w-xl">
                ...
                <p className="text-xs text-slate-400 leading-relaxed">
                  Ao iniciar o expediente, realize a abertura informando o saldo
                  inicial da gaveta física. Durante o dia, qualquer compra de
                  materiais ou venda balcão em dinheiro físico altera o saldo de
                  forma controlada. No fim do expediente, conte o numerário
                  físico e feche o caixa para registrar e conciliar
                  automaticamente quaisquer quebras ou sobras.
                </p>
              </div>
                ...
                <table className="w-full text-left text-xs text-slate-400">
                  <thead className="sticky top-0 bg-slate-900 border-b border-slate-800/80 text-slate-500 font-mono text-[10px] uppercase z-10">
                    <tr>
                      ...
                      <th className="py-2.5 px-3 text-right">Contagem Real</th>
          ...
          <div className="px-6 py-4 border-b border-slate-800 bg-slate-950/15 flex flex-col xl:flex-row xl:items-center justify-between gap-4">
            <div>
              <h3 className="font-bold text-slate-100">
                Razão Auxiliar / Extrato de Operações
              </h3>
        ...
        {isExpenseModalOpen && (
          <ExpenseModal setIsOpen={setIsExpenseModalOpen} />
        )}
        {isUndoModalOpen && (
