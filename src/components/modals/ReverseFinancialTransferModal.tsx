import { useState, type FormEvent } from "react";
import { AlertCircle, ArrowRightLeft, RotateCcw, X } from "lucide-react";
import { motion } from "motion/react";
import { useReverseFinancialTransfer } from "../../utils/queries";

export type ReversibleFinancialTransfer = {
  id: number;
  description: string;
  sourceAccount: string;
  targetAccount: string;
  value: number;
};

type Props = {
  item: ReversibleFinancialTransfer;
  setIsOpen: (open: boolean) => void;
};

export default function ReverseFinancialTransferModal({
  item,
  setIsOpen,
}: Props) {
  const reverseTransfer = useReverseFinancialTransfer();
  const [reason, setReason] = useState("");
  const [error, setError] = useState("");

  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    setError("");
    if (reason.trim().length < 3 || reason.trim().length > 30) {
      setError("Informe um motivo entre 3 e 30 caracteres.");
      return;
    }
    try {
      await reverseTransfer.mutateAsync({
        transferID: item.id,
        reason: reason.trim(),
      });
      setIsOpen(false);
    } catch (requestError) {
      const apiError = requestError as {
        mensagem?: string;
        response?: { data?: { mensagem?: string } };
      };
      setError(
        apiError.mensagem ??
          apiError.response?.data?.mensagem ??
          "Não foi possível estornar a transferência.",
      );
    }
  }

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/75 p-4 backdrop-blur-sm">
      <motion.div
        initial={{ opacity: 0, scale: 0.95 }}
        animate={{ opacity: 1, scale: 1 }}
        className="w-full max-w-md overflow-hidden rounded-2xl border border-slate-800 bg-slate-900 shadow-2xl"
      >
        <div className="flex items-center justify-between border-b border-slate-800 px-6 py-4">
          <div className="flex items-center gap-2">
            <RotateCcw className="h-5 w-5 text-rose-400" />
            <h3 className="font-bold text-slate-100">Estornar transferência</h3>
          </div>
          <button
            type="button"
            onClick={() => setIsOpen(false)}
            className="rounded-lg p-1.5 text-slate-400 hover:bg-slate-800 hover:text-slate-100"
          >
            <X className="h-5 w-5" />
          </button>
        </div>
        <form onSubmit={handleSubmit} className="space-y-4 p-6">
          <div className="flex gap-2 rounded-xl border border-rose-500/20 bg-rose-500/5 p-3 text-xs text-rose-200">
            <AlertCircle className="h-4 w-4 shrink-0" />
            <p>
              O estorno devolverá o valor à conta de origem e removerá o crédito
              da conta de destino.
            </p>
          </div>
          <div className="space-y-2 rounded-xl border border-slate-800 bg-slate-950/40 p-4 text-xs">
            <p className="font-bold text-slate-200">
              #{item.id} — {item.description}
            </p>
            <p className="flex items-center gap-2 text-slate-400">
              <span>{item.sourceAccount}</span>
              <ArrowRightLeft className="h-3.5 w-3.5 text-sky-400" />
              <span>{item.targetAccount}</span>
            </p>
            <p className="font-mono text-sm font-bold text-slate-100">
              R${" "}
              {item.value.toLocaleString("pt-BR", { minimumFractionDigits: 2 })}
            </p>
          </div>
          <label className="block text-[10px] font-bold uppercase text-slate-400">
            Motivo do estorno
            <textarea
              required
              minLength={3}
              maxLength={30}
              rows={3}
              value={reason}
              onChange={(event) => setReason(event.target.value)}
              className="mt-1.5 w-full resize-none rounded-xl border border-slate-800 bg-slate-950/40 p-3 text-xs normal-case text-slate-100"
            />
          </label>
          {error && <p className="text-xs text-rose-400">{error}</p>}
          <div className="flex justify-end gap-2 border-t border-slate-800 pt-4">
            <button
              type="button"
              disabled={reverseTransfer.isPending}
              onClick={() => setIsOpen(false)}
              className="rounded-xl bg-slate-800 px-4 py-2 text-xs font-bold uppercase text-slate-300 disabled:opacity-50"
            >
              Voltar
            </button>
            <button
              type="submit"
              disabled={reverseTransfer.isPending}
              className="rounded-xl bg-rose-500 px-4 py-2 text-xs font-bold uppercase text-slate-950 disabled:opacity-50"
            >
              {reverseTransfer.isPending
                ? "Estornando..."
                : "Confirmar estorno"}
            </button>
          </div>
        </form>
      </motion.div>
    </div>
  );
}
