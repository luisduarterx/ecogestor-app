...
import { useCreateUser, useUpdateUser } from "../../utils/queries";
import type {
  ApiError,
  RolesResponse,
...
export default function NewUserModal({
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setError(
        (caught as ApiError).mensagem ?? "Não foi possível salvar o usuário.",
      );
    }
  }
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">
                Informe os dados de acesso e atribua um cargo. As permissões são
                definidas pelo cargo.
              </p>
            </div>
          </div>
          <button
        ...
        <form onSubmit={(event) => void submit(event)}>
          ...
          <div className="flex items-center justify-end gap-3 border-t border-slate-800 bg-slate-950/60 p-4">
            <button
              ...
              onClick={onClose}
              disabled={pending}
              className="rounded-xl bg-slate-800 px-4 py-2 text-xs font-bold uppercase text-slate-300 hover:bg-slate-700"
            >
              Cancelar
            </button>
            <button
